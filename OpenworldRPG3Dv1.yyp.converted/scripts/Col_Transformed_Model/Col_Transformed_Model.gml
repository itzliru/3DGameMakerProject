...
function ColTransformedModel(mesh, position = new Vector3(0, 0, 0), rotation = matrix_build_identity()) constructor {
    ...
    static Set = function(mesh = self.mesh, position = self.position, rotation = self.rotation) {
        // Local transform matrix for position
        var position_mat = matrix_build(position.x, position.y, position.z, 0, 0, 0, 1, 1, 1);
        ...
        // Use matrix_multiply/ matrix_inverse return values (new GM API)
        self.property_transform = matrix_multiply(rotation, position_mat);
        self.property_inverse = matrix_inverse(self.property_transform);
    ...
    static CheckOBB = function(obb) {
        ...
        // Compute combined inverse for the OBB orientation using matrix API that returns matrices
        var untransformed_inverse = matrix_multiply(obb.orientation, self.property_inverse);
