...
function mat4_mul_point(mat, point) {
    ...
    // `matrix_transform_vertex` accepts (matrix, x, y, z, out_array)
    matrix_transform_vertex(mat, point.x, point.y, point.z, transformed_point);
    return new Vector3(transformed_point[0], transformed_point[1], transformed_point[2]);
}

function mat4_mul_vector(mat, vec) {
    static transformed_point = [0, 0, 0, 0];
    // vectors use w == 0 implicitly; pass output array only
    matrix_transform_vertex(mat, vec.x, vec.y, vec.z, transformed_point);
    return new Vector3(transformed_point[0], transformed_point[1], transformed_point[2]);
}